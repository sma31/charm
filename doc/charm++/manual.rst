...
allows the receiver to specify the memory registration mode. This is done by indexing the ncpyPost array
and assigning the mode parameter present inside each array element of the ``CkNcpyBufferPost`` array.
When the network memory registration mode is unassigned by the user, the default ``CK_BUFFER_REG`` mode is used.
...
.. code-block:: c++
   ...
   void foo (int &size, int *& arr, CkNcpyBufferPost *ncpyPost) {
     ...
     ncpyPost[0].mode = CK_BUFFER_REG; // specify the mode for the 0th pointer
   }
...
entry methods with nocopypost specified parameters is currently supported for
